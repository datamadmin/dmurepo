...
@Service
@Slf4j
public class DmuScriptGenerationService {
			...
			if (dmuHistoryDetailListOpt.isPresent()) {
				dmuHistoryDetailListOpt.get().stream().forEach(dmuHistoryDetail -> {
					...
					if (StringUtils.isBlank(dmuHistoryDetail.getFilterCondition())
							...
							&& DmuConstants.YES.equalsIgnoreCase(connectionDto.getTgtFormatPropDto().getSrcFormatFlag())) {
	...
	private void populateTGTOtherProperties(DmuConnectionDTO connectionDto, Optional<DmuTgtOtherPropEntity> tgtOtherProp) {
		if (tgtOtherProp.isPresent()) {
			DmuTgtOtherPropEntity tgtOtherPropObj = tgtOtherProp.get();
			connectionDto.setTgtOtherPropDto(DmuTgtOtherPropDTO.builder().parallelJobs(tgtOtherPropObj.getParallelJobs())
	...
	private void populateTGTFormatProperties(DmuConnectionDTO connectionDto, Optional<DmuTgtFormatEntity> tgtFormatProp) {
